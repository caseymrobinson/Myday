...
export default function TasksPanel({ tasks, isLoading, onAddTask, onSetupCalendar, onPlanDay }: TasksPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-black">
      ...
      {/* Tasks Container */}
      <div className="flex-1 overflow-y-auto px-6 bg-[#1E1E1E] text-[#9ca3af]">
        {isLoading ? (
              ...
              <div className="space-y-2">
                {todoTasks.length === 0 ? (
                  ...
                  todoTasks.map((task) => (
                    <div
                      key={task.id}
                      className="rounded-xl p-4 hover:bg-gray-800 transition-colors bg-[#2D2D2D] pl-[14px] pr-[14px] pt-[14px] pb-[14px]"
